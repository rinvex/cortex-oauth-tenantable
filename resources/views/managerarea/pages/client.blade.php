...
@push('inline-scripts')
    {!! JsValidator::formRequest(Cortex\Oauth\Http\Requests\Managerarea\ClientFormRequest::class)->selector("#managerarea-cortex-oauth-create-form, #managerarea-cortex-oauth-{$client->getRouteKey()}-update-form")->ignore('.skip-validation') !!}
@endpush

{{-- Main Content --}}
@section('content')

    @includeWhen($client->exists, 'cortex/foundation::managerarea.partials.modal', ['id' => 'delete-confirmation'])
    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            ...
            <div class="nav-tabs-custom">
                ...
                @if($client->exists && (request()->user()->can('revoke', $client) || request()->user()->can('delete', $client) || request()->user()->can('create', $client)))
                    <div class="pull-right">
                        ...
                        @if (request()->user()->can('create', $client))
                            <a href="{{ route('managerarea.cortex.oauth.clients.create', ['replicate' => $client->getRouteKey()]) }}" title="{{ trans('cortex/foundation::common.replicate') }}" class="btn btn-default" style="margin: 4px"><i class="fa fa-clone text-default"></i></a>
                        @endif

                        @if(request()->user()->can('revoke', $client))
                            <a href="#" data-toggle="modal" data-target="#delete-confirmation"
                               data-modal-action="{{ route('managerarea.cortex.oauth.clients.revoke', ['client' => $client]) }}"
                        ...
                        @if(request()->user()->can('delete', $client))
                            <a href="#" data-toggle="modal" data-target="#delete-confirmation"
                               data-modal-action="{{ route('managerarea.cortex.oauth.clients.destroy', ['client' => $client]) }}"
                               data-modal-title="{{ trans('cortex/foundation::messages.delete_confirmation_title') }}"
                ...
                {!! Menu::render('managerarea.cortex.oauth.clients.tabs', 'nav-tab') !!}
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane active" id="details-tab">
                            ...
                            @if ($client->exists)
                                {{ Form::model($client, ['url' => route('managerarea.cortex.oauth.clients.update', ['client' => $client]), 'method' => 'put', 'id' => "managerarea-cortex-oauth-clients-{$client->getRouteKey()}-update-form", 'files' => true]) }}
                            @else
                                {{ Form::model($client, ['url' => route('managerarea.cortex.oauth.clients.store'), 'id' => 'managerarea-cortex-oauth-clients-create-form', 'files' => true]) }}
                            @endif
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    @include('cortex/foundation::managerarea.partials.timestamps', ['model' => $client])
