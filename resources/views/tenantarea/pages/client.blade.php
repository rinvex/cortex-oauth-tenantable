...
@push('inline-scripts')
    {!! JsValidator::formRequest(Cortex\Oauth\Http\Requests\Tenantarea\ClientFormRequest::class)->selector("#tenantarea-cortex-oauth-clients-create-form, #tenantarea-cortex-oauth-clients-{$client->getRouteKey()}-update-form")->ignore('.skip-validation') !!}
@endpush

{{-- Main Content --}}
@section('content')

    @includeWhen($client->exists, 'cortex/foundation::tenantarea.partials.modal', ['id' => 'delete-confirmation'])
            ...
            <div class="col-md-9">
                ...
                <div class="profile-content">
                    ...
                    <div class="nav-tabs-custom">
                        {!! Menu::render('tenantarea.cortex.oauth.clients.tabs', 'nav-tab') !!}

                        <div class="tab-content">

                            <div class="tab-pane active" id="tenantarea-cortex-oauth-clients-{{ $client->exists ? $client->getRouteKey().'-' : '' }}tab">

                                @if ($client->exists)
                                    {{ Form::model($client, ['url' => route('tenantarea.cortex.oauth.clients.update', ['client' => $client]), 'method' => 'put', 'id' => "tenantarea-cortex-oauth-clients-{$client->getRouteKey()}-update-form", 'files' => true]) }}
                                @else
                                    {{ Form::model($client, ['url' => route('tenantarea.cortex.oauth.clients.store'), 'id' => 'tenantarea-cortex-oauth-clients-create-form', 'files' => true]) }}
                                @endif
                                    ...
                                    <div class="col-md-12 text-center profile-buttons">
                                        ...
                                        @if($client->exists && request()->user()->can('revoke', $client))
                                            <a href="#" data-toggle="modal" data-target="#delete-confirmation"
                                               data-modal-action="{{ route('tenantarea.cortex.oauth.clients.revoke', ['client' => $client]) }}"
                                        ...
                                        @if($client->exists && request()->user()->can('delete', $client))
                                            <a href="#" data-toggle="modal" data-target="#delete-confirmation"
                                               data-modal-action="{{ route('tenantarea.cortex.oauth.clients.destroy', ['client' => $client]) }}"
                                               data-modal-title="{{ trans('cortex/foundation::messages.delete_confirmation_title') }}"
