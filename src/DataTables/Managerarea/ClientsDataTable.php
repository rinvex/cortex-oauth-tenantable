...
class ClientsDataTable extends AbstractDataTable
{
    ...
    {
        return config('cortex.foundation.route.locale_prefix')
            ? '"<a href=\""+routes.route(\'managerarea.cortex.oauth.clients.edit\', {client: full.id, locale: \''.$this->request()->segment(1).'\'})+"\">"+data+"</a>"'
            : '"<a href=\""+routes.route(\'managerarea.cortex.oauth.clients.edit\', {client: full.id})+"\">"+data+"</a>"';
    }
